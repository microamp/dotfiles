...
(global-set-key (kbd "C-c C-SPC") #'microamp/kill-pwd)
...
(use-package eshell
  ...
  (defun microamp/eshell-clear ()
    ;; NOTE: This erases shell content!
    (interactive)
    (let ((inhibit-read-only t))
      (erase-buffer))
    (eshell-send-input))
  (defun microamp/eshell-init ()
    (define-key eshell-mode-map (kbd "C-l") #'microamp/eshell-clear))
  (add-hook 'eshell-mode-hook #'microamp/eshell-init))
...
(use-package project
  ...
  :bind (:map project-prefix-map
              ...
              ("s" . consult-ripgrep))
  :config
  (defun microamp/project-magit-status ()
    (interactive)
    (magit-status (project-root (project-current))))
  (add-to-list 'project-switch-commands '(microamp/project-magit-status "Magit" "m"))
  (add-to-list 'project-switch-commands '(consult-ripgrep "Ripgrep" "q")))

(use-package pulse
  :custom
  (pulse-delay 0.03)
  :init
  (defun microamp/pulse (&rest args)
  ...
  (let ((pulse-commands '(delete-other-windows
    ...
    (dolist (command pulse-commands)
      (advice-add command :after #'microamp/pulse))))

;; Dependencies:
;; python -m pip install --user --upgrade python-lsp-server
;; python -m pip install --user --upgrade pyright
...
(use-package treesit
  ...
  (defun microamp/treesit-install-all-languages ()
    (interactive)
...
(use-package window
  :config
  (defun microamp/focus-other-window (&rest args)
    (other-window 1))
  (advice-add 'split-window-horizontally :after #'microamp/focus-other-window)
  (advice-add 'split-window-vertically :after #'microamp/focus-other-window))
...
(use-package kanagawa-theme
  :ensure
  :hook ((window-setup . (lambda () (load-theme 'kanagawa))))
  :custom
  (kanagawa-theme-org-bold t)
  (kanagawa-theme-org-height nil)
  (kanagawa-theme-org-priority-bold t)
  (kanagawa-theme-underline-parens nil)
  :init
  (custom-set-faces
   `(org-block
     ((t (:inherit org-code :background nil :foreground nil))))))
...
(use-package magit
  ...
  (advice-add 'magit-show-refs-head :after #'(lambda (&rest args) (delete-other-windows)))
  (advice-add 'magit-status :before #'(lambda (&rest args) (save-buffer)))
